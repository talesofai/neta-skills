...
const errorsYamlFiles = await glob(
  join(sourceDir, "**/utils/errors.*.yml"),
).then((files) => files.map((file) => file.replace(sourceDir, "")));
...
for (const errorsYamlFile of errorsYamlFiles) {
  await cp(join(sourceDir, errorsYamlFile), join(distDir, errorsYamlFile));
}
