...
export const getFavorList = createCommand(
  ...
  async ({ parent_type, page_index, page_size }, { log, apis }) => {
    log.debug(
      "get_favor_list: parent_type: %s, page_index: %d, page_size: %d",
      parent_type,
      page_index,
      page_size,
    );

    const apiParentType: "oc" | "elementum" | undefined = parent_type === "both" ? undefined : (parent_type === "character" ? "oc" : "elementum");
