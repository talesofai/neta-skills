...
export const apiClient = {
  ...
  async pollTask(
    taskUuid: string,
    timeout: number = 10 * 60 * 1000,
  ): Promise<{ result: TaskResult; isTimeout: false } | { result: null; isTimeout: true }> {
    return polling(
      () => this.task(taskUuid),
      (result) => {
  ...
  async makeImage(params: MakeImageV1Parameters): Promise<TaskResult> {
    ...
    const pollResult = await this.pollTask(taskUuid, 10 * 60 * 1000);
    if (pollResult.isTimeout || !pollResult.result) {
  ...
  async makeVideo(params: MakeVideoV1Parameters): Promise<TaskResult> {
    ...
    const taskUuid = res.data;
    const pollResult = await this.pollTask(taskUuid, 20 * 60 * 1000);
    if (pollResult.isTimeout || !pollResult.result) {
      throw new Error('Task timeout');
    }
    return pollResult.result;
  ...
  async makeSong(params: MakeSongV1Parameters): Promise<TaskResult> {
    ...
    const taskUuid = res.data;
    const pollResult = await this.pollTask(taskUuid, 5 * 60 * 1000);
    if (pollResult.isTimeout || !pollResult.result) {
      throw new Error('Task timeout');
    }
    return pollResult.result;
  ...
  async searchTCPs(
    params: SearchCharacterOrElementumV1Parameters,
  ): Promise<SearchCharacterOrElementumV1Result> {
