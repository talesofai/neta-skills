...
async function fetchStories(hashtag, pageIndex, pageSize = 20, sortBy = "newest") {
  ...
  console.log(`   URL: ${url}`);
  console.log(`   编码后：${encodeURIComponent(hashtag)}`);
...
async function main() {
  // 从重定向 URL 提取的正确 hashtag
  const hashtag = "捏 ta 有个挺牛的宿社";
  
  console.log(`🔍 测试 hashtag: "${hashtag}"`);
  console.log(`   长度：${hashtag.length}`);
  console.log(`   编码：${encodeURIComponent(hashtag)}`);
