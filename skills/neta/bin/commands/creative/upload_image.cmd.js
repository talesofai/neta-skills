...
const canonicalSuffix = (suffix) => suffix.toLowerCase() === "jpeg" ? "jpg" : suffix.toLowerCase();
const normalizeSuffix = (suffix) => {
    if (!suffix)
        return null;
    const value = suffix.replace(/^\./, "").toLowerCase();
    if (!ALLOWED_SUFFIXES.has(value))
        return null;
    return value;
};
...
const resolveSource = (args) => {
    const picked = SOURCE_KEYS.filter((key) => {
        const value = args[key];
        return typeof value === "string" && value.trim().length > 0;
    });
    if (picked.length === 0) {
        throw new Error("E_MISSING_INPUT: 至少提供一个图片输入参数(image/input/image_path/image_url/image_data)");
    }
    const key = picked[0];
    if (!key) {
...
const readInputBytes = async (key, value) => {
    if (key === "image_data") {
    ...
    if (key === "image_url" || /^https?:\/\//i.test(value)) {
        ...
        if (!response.ok) {
            throw new Error(`E_DOWNLOAD_FAILED: 下载图片失败 status=${response.status} url=${value}`);
        }
...
const inferContentType = (args) => {
    ...
    if (explicit) {
        if (!explicit.startsWith("image/")) {
            throw new Error("E_UNSUPPORTED_CONTENT_TYPE: content_type 必须是 image/*");
        }
        return explicit;
    }
    ...
    throw new Error("E_CANNOT_INFER_CONTENT_TYPE: 无法识别图片类型，请显式传入 content_type 或 suffix");
};
const inferSuffix = (args) => {
    ...
    const inferred = byParam ?? byFilename ?? bySourceName ?? byContentType ?? byBytes;
    if (!inferred) {
        throw new Error("E_CANNOT_INFER_SUFFIX: 无法识别图片后缀，请显式传入 suffix 或 filename");
    }
    if (byParam && byContentType) {
        const sameType = canonicalSuffix(byParam) === canonicalSuffix(byContentType);
        if (!sameType) {
            throw new Error(`E_SUFFIX_CONTENT_TYPE_CONFLICT: suffix=${byParam} 与 content-type=${byContentType} 冲突`);
        }
    }
    return inferred;
};
...
export const uploadImage = createCommand({
    ...
    if (source.hasAmbiguousInput) {
        log.warn("upload_image: multiple input aliases provided, use key=%s", source.key);
    }
    ...
    log.info("upload_image: prepare trace=%s key=%s sourceType=%s bytes=%d suffix=%s contentType=%s", traceId, source.key, inputData.sourceType, inputData.bytes.length, suffix, contentType);
    ...
    const structuredContent = {
        status: "uploaded",
        source_type: inputData.sourceType,
    ...
    log.info("upload_image: result trace=%s path=%s", traceId, structuredContent.path);
