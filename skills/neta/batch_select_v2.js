...
// 标记帖子为精选
async function markAsFeatured(encodedHashtag, storyUuid) {
  const url = `${BASE_URL}/v1/hashtag/${encodedHashtag}/stories/review/${storyUuid}`;
...
async function main() {
  ...
  const encodedHashtag = encodeURIComponent(hashtag).replace(/%20/g, ''); // 移除空格编码
  const targetCount = 30;
  
  console.log(`🔍 开始处理 hashtag: "${hashtag}"`);
  console.log(`   编码：${encodedHashtag}`);
  ...
  while (hasMore && allStories.length < targetCount) {
    ...
    const result = await fetchStoriesRaw(encodedHashtag, page, pageSize, "newest");
    ...
    console.log(`   本页获取到 ${result.list.length} 个`);
  ...
  if (needSelect.length > 0) {
    ...
    for (const story of needSelect) {
      try {
        const result = await markAsFeatured(encodedHashtag, story.id);
