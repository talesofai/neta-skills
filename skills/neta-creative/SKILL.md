...
1. For tasks that **create or edit concrete assets** (images, videos, songs, MVs, background removal), follow this flow:
   ...
   - **Validation checkpoints for multi‑step workflows (e.g. MV creation):** After each generation step, verify the output before proceeding. Confirm the song was created successfully before designing visuals; confirm the image URL is valid before passing it to `make_video`; confirm each video clip rendered before assembling the final MV. If any step produces an unexpected result, pause and resolve it before continuing.
...
3. **Error recovery for common failures:**
   - **Quota / credit limit (电量不足):** If a generation call fails due to insufficient credits, inform the user of the reason and offer the premium upgrade path once (list plans → create order → pay). Do not retry the generation until credits are available.
   - **Rate / frequency limit (频率限制):** If a call is rejected for exceeding usage frequency, wait briefly and retry once. If it fails again, inform the user of the cooldown period and suggest resuming shortly.
   - **Generation failure or timeout:** If `make_image`, `make_video`, or `make_song` returns an error or times out, retry the same call once with identical parameters. If it fails a second time, report the error to the user and suggest adjusting the prompt or trying a different model (e.g. `model_s` instead of `model_w`).
4. **Premium** (plans, orders, Stripe): use the commands below. Run **`get_current_premium_plan`** before and after checkout so the user can confirm tier (and end time if returned). Commerce needs the **global** Neta API—see the premium reference. If creation is blocked by **quota / credits (电量)** or **usage frequency (频率)**, say why in one beat; if an upgrade fits their goal, offer the path once (list plans → create order → pay)—**do not** repeat upgrade nudges in the same conversation unless the user asks.
