...
```bash
npx -y @talesofai/neta-skills suggest_keywords --prefix "游戏" --size 20
```
...
```bash
npx -y @talesofai/neta-skills suggest_tags --keyword "角色塑造" --size 15
```
...
# 获取二级分类（需要父级路径）
npx -y @talesofai/neta-skills suggest_categories --level 2 --parent_path "衍生创作类"

# 获取三级分类（最细粒度）
npx -y @talesofai/neta-skills suggest_categories --level 3 --parent_path "衍生创作类>同人二创"
```
...
```bash
npx -y @talesofai/neta-skills validate_tax_path --tax_path "衍生创作类>热门 IP>崩坏星穹铁道"
```
...
```bash
# 基于关键词找标签
npx -y @talesofai/neta-skills suggest_tags --keyword "同人二创" --size 15
# 输出示例：["崩坏星穹铁道", "原神", "明日方舟"]

# 或者用关键词建议辅助
npx -y @talesofai/neta-skills suggest_keywords --prefix "崩" --size 10
...
```bash
# 步骤 1：先用关键词建议发现方向
npx -y @talesofai/neta-skills suggest_keywords --prefix "游" --size 15

# 步骤 2：基于发现的关键词找标签
npx -y @talesofai/neta-skills suggest_tags --keyword "游戏" --size 15
...
```bash
# 步骤 1：了解热门标签
npx -y @talesofai/neta-skills suggest_tags --keyword "角色塑造" --size 20
...
```bash
# 第一次请求
npx -y @talesofai/neta-skills suggest_categories --level 1 > /tmp/categories_level1.json
...
npx -y @talesofai/neta-skills suggest_categories --level 2 --parent_path "衍生创作类" &
npx -y @talesofai/neta-skills suggest_categories --level 2 --parent_path "数字艺术" &
npx -y @talesofai/neta-skills suggest_categories --level 2 --parent_path "生活方式" &
wait
```
...
```bash
# 并行验证多个路径
npx -y @talesofai/neta-skills validate_tax_path --tax_path "衍生创作类>同人二创>崩坏星穹铁道" &
npx -y @talesofai/neta-skills validate_tax_path --tax_path "衍生创作类>同人二创>原神" &
npx -y @talesofai/neta-skills validate_tax_path --tax_path "数字艺术>概念设计" &
wait
...
while read path; do
  npx -y @talesofai/neta-skills validate_tax_path --tax_path "$path"
done < /tmp/paths.txt
```
...
```bash
# 开启 debug 日志
DEBUG=* npx -y @talesofai/neta-skills suggest_content \
  --intent search \
...
```bash
# 同一关键词，对比三种模式
npx -y @talesofai/neta-skills suggest_content --intent recommend --page_size 10
npx -y @talesofai/neta-skills suggest_content --intent search --search_keywords "关键词" --page_size 10
npx -y @talesofai/neta-skills suggest_content --intent exact --tax_paths "分类路径" --page_size 10
```
...
```bash
# keywords - 前缀匹配
npx -y @talesofai/neta-skills suggest_keywords --prefix "崩"  # 返回所有以"崩"开头的词

# tags - 相关性匹配
npx -y @talesofai/neta-skills suggest_tags --keyword "游戏"   # 返回与"游戏"相关的标签
```
